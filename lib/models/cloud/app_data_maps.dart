...
abstract final class AppDataMaps {
    ...
    'attachments': a.attachments
        .map(
          (att) => {
            ...
            if (att.fileSizeBytes != null) 'fileSizeBytes': att.fileSizeBytes,
          },
        )
        .toList(),
  ...
  static Announcement announcementFromMap(Map<String, dynamic> map) =>
      Announcement(
        ...
        date: DateTime.parse(map['date'] as String),
        audienceKeys:
            (map['audienceKeys'] as List<dynamic>?)
                ?.map((e) => e.toString())
                .toList() ??
  ...
  static TransportPassengerStatus transportPassengerStatusFromName(
    String? raw,
  ) {
