-- LIA Phase G: student support + child-protection reads.
-- Replaces app_doc_readable (and tightens teacher writes). Copies Phase E
-- rules, then gates safeguarding_cases before jwt_can_read_all_school_data()
-- so QA, classroom teachers, parents, and students never read CP files.
-- Parents read own-child health/counseling/IEP/college. Students read own
-- college plan + own support_requests only.
